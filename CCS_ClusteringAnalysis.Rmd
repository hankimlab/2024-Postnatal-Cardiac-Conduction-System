...
```{r}
allcells <- merge(x=p1, y=c(p2,p4), add.cell.ids = c("P1", "P2", "P4"), project = "ccs")
```

# Pre-processing - define percent.mito & filter cells based on nFeature & percent.mito
```{r}
allcells[["percent.mito"]] <-
  PercentageFeatureSet(allcells, pattern = "^mt-")
...
allcells <- FindVariableFeatures(allcells, selection.method = "vst", nfeatures = 5000)
allcells <- ScaleData(allcells, features = rownames(allcells))
allcells <- RunPCA(allcells, npcs = 50)
allcells <- RunUMAP(allcells, dims = 1:25) 
allcells <- FindNeighbors(allcells, dims = 1:25)
allcells <- FindClusters(allcells, resolution = 0.25)
DimPlot(allcells)
```

# Extract cardiomyocytes (CMs) & perform seurat workflow
```{r}
ccs <- subset(allcells, idents = c('0', '1', '2', '3', '4'))
